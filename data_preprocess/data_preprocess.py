...
parser.add_argument('-r', '--raw-dir', action='store', nargs='?', default='raw_dir', dest='raw_dir', help="Raw image directory for augmentation")
parser.add_argument('-o', '--output-dir', action='store', nargs='?', default='output_dir', dest='output_dir', help="Output image directory to images gonna save")
parser.add_argument('-a', '--contrast', type=float, action='store', nargs='?', default=1, dest='contrast', help="Contrast settings for low contrast image")
parser.add_argument('-b', '--brightness', type=int, action='store', nargs='?', default=0, dest='brightness', help="Brightness settings for low contrast image")
...
fn = [join(parameters.raw_dir, f) for f in listdir(dir_raw_img)]
...
for folder in tqdm(fn, desc='Folder', unit='fold'):
    ...
    j_sum = 0
    i = 0
    for i, img_path in enumerate(tqdm(filename_raw[::10], desc='Image(s) in {} folder'.format(fd), unit='img', position=0)):
        j = 0
